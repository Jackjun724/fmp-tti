...
function checkNodeScore(node: HTMLElement): number {
    ...
    if (node !== document.body) {
        ...
        if (y < windowHeight && width > 0 && height > 0) {
            if (node.tagName !== 'IMG') {
...
/** 获取图片/视频节点生效的事件名 */
function getMediaEventName(tagName: string) {
    return tagName == 'IMG' ? 'load' : 'canplay';
}
 ...
 * @param {Event} event
 */
function addMediaScore(this: HTMLImageElement) {
    addScore(checkNodeScore(this));
    this.removeEventListener(getMediaEventName(this.tagName), addMediaScore);
}
...
function checkNodeList(nodes: NodeList): number {
    ...
    for (let i = 0, l = nodes.length; i < l; i++) {
        const node = nodes[i];
        const { tagName } = <HTMLElement>node;
        if (tagName === 'IMG' || tagName === 'VIDEO') {
            node.addEventListener(getMediaEventName(tagName), addMediaScore);
